...
std::vector<std::complex<double>> rk4(
    ...
    auto k1 = derivadaVectorPsi(state, oper, num_procs);
	...
	auto k2 = derivadaVectorPsi(state_k2, oper, num_procs);
	
	for (size_t i = 0; i < state.size(); ++i) {
		state_k3[i] = state[i] + 0.5 * h * k2[i];
    }

	auto k3 = derivadaVectorPsi(state_k3, oper, num_procs);
	
	for (size_t i = 0; i < state.size(); ++i) {
		state_k4[i] = state[i] + h * k3[i];
    }

	auto k4 = derivadaVectorPsi(state_k4, oper, num_procs);
...
int main() {
    ...
    for (int i = 0; i < TIMESTEPS; ++i) {
        ...
        vecRK4 = rk4(vecRK4, H, h, num_procs);
    }
